...
class Exam_subs extends CI_Model
{
	...
	{
		$query = $this->db->query("Select * from live_subz_exam");
		return $query->result();
	}
	...
	public function b4_clean(){
		$this->db->query("insert into daily_report_exam (cur_date,total_succeded,total_failed) values (curdate(),(select count(user_number) FROM live_subz_exam WHERE reponse_val = 'Success'),(select count(user_number) FROM live_subz_exam WHERE reponse_val <> 'Success'))");
	}
	public function clean(){
		$this->db->query("update live_subz_exam set reponse_val = null where 1");
	}

	public function chk_daily_report()
	{
		$this->db->from("daily_report_exam");
